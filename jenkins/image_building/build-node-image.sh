...
export TMP_HOOKS_PATH="${current_dir}/tmp_hooks"
...
node_img_name="metal3node-${IMAGE_OS}-${img_date}-${commit_short}"

# Create an image
disk-image-create --no-tmpfs -a amd64 -o "${node_img_name}".qcow2 "${IMAGE_OS}"-node block-device-efi

# Keep latest N number of images and delete old ones

# Push image to openstack Kna region
openstack image create "${node_img_name}" --file "${node_img_name}".qcow2 --disk-format=qcow2
