...
def UPDATED_REPO, BUILD_TAG, GINKGO_SKIP, CURRENT_END_TIME, CURRENT_START_TIME
            ...
            matrix {
                ...
                stages {
                    stage('Init') {
                        steps {
                             script {
                                 CURRENT_DIR = sh(script: 'readlink -f "."', returnStdout: true).trim()
                    ...
                    stage('Test IPA with dev-env'){
                      steps {
                          withCredentials([string(credentialisId: 'metal3-clusterctl-github-token', variable: 'GITHUB_TOKEN')]) {
                             /* Fullstack building script */
                            timestamps {
                                sh './jenkins/scripts/dynamic_worker_workflow/ipa_dev_env_test.sh'
                            }
                         }
                      }
                   }
                   stage('IPA BMO e2e test placeholder'){
                      steps {
                         withCredentials(
                            [string(credentialisId: 'metal3-clusterctl-github-token', variable: 'GITHUB_TOKEN')]) {
                            /* Fullstack building script */
                            timestamps {
                              echo 'Placeholder, test is not yet implemented'
                            }
                         }
                      }
                   }
                   stage('Artifactory upload'){
                      steps {
                         withCredentials([usernamePassword(credentialsId: 'infra-nordix-artifactory-api-key', usernameVariable: 'RT_USER', passwordVariable: 'RT_TOKEN'), 
                            ...
                            /* Fullstack building script */
                            timestamps {
                                sh './jenkins/scripts/dynamic_worker_workflow/ipa_artifact_upload.sh'
                            }
                         }
                      }
                   }
