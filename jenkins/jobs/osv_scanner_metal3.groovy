...
script {
    ci_git_branch = (env.PULL_PULL_SHA) ?: "add-osv-scanner-bmo/adil"
    ci_git_base = (env.PULL_BASE_REF) ?: "main"
    ci_git_url = "https://github.com/Nordix/project-infra.git"
...
def CAPM3_GOPROXY = "https://proxy.golang.org/github.com/metal3-io/cluster-api-provider-metal3"
def BMO_GOPROXY  = "https://proxy.golang.org/github.com/metal3-io/baremetal-operator"
def IPAM_GOPROXY = "https://proxy.golang.org/github.com/metal3-io/ip-address-manager"
def IRSO_GOPROXY = "https://proxy.golang.org/github.com/metal3-io/ironic-standalone-operator"

def CAPM3_GIT_URL = "https://github.com/metal3-io/cluster-api-provider-metal3.git"
def BMO_GIT_URL   = "https://github.com/metal3-io/baremetal-operator.git"
def IPAM_GIT_URL  = "https://github.com/metal3-io/ip-address-manager.git"
def IRSO_GIT_URL  = "https://github.com/metal3-io/ironic-standalone-operator.git"
...
pipeline {
    ...
    stages {
        ...
        stage('Install OSV-Scanner') {
            ...
            steps {
                script {
                    echo "Running external install scripts for Go ${GO_VERSION} and OSV-Scanner"
                    sh """
                      curl -sSfL https://go.dev/dl/"go${GO_VERSION}".linux-amd64.tar.gz -o go.tar.gz
                      sudo rm -rf /usr/local/go && sudo tar -C /usr/local -xzf go.tar.gz
                      rm go.tar.gz
                      export PATH=\$PATH:/usr/local/go/bin
                      # Installing OSV-Scanner version v2.2.4
                      go install github.com/google/osv-scanner/v2/cmd/osv-scanner@8b6727b2c439cdea8bc3a033bf7c76d76cbaee08
                    """
                }
                sh 'mkdir -p results'
            }
                    ...
                    entry.branches.each { br ->
                        ...
                        tasks[label] = {
                            ...
                            dir(workDir) {
                                ...
                                sh "osv-scanner --config ./config.toml ${params.SCAN_ARGS} --output ${outFile} ."
                            }
                        }
                    }
                }
