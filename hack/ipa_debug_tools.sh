...
BASE_HELPER_FILE_DIR=$(mktemp -dp "${BASE_DEBUG_DIR}")
...
set_up_debug_dirs() {
    ...
    mkdir -p "${BASE_DEBUG_DIR}/build-ipa-initramfs"
    cp /tmp/dib/ironic-python-agent.initramfs "${BASE_DEBUG_DIR}/"
    cd "${BASE_DEBUG_DIR}/build-ipa-initramfs"
    gunzip -c ../ironic-python-agent.initramfs | cpio -id

    # Get master ironic-python-agent.initramfs and unzip
    mkdir -p "${BASE_DEBUG_DIR}/master-ipa-initramfs"
    cd "${BASE_DEBUG_DIR}"
    wget "${full_path}"
    tar -xzf "${file_name}"
    cd master-ipa-initramfs
    gunzip -c "../${file_base_name}.initramfs" | cpio -id
}
...
clean_up_debug_dirs() {
    ...
    sudo rm -rf "${BASE_DEBUG_DIR}" /tmp/dib /tmp/ipa-file-injector.service 2> /dev/null
}

# Clean up all helper temp files.
clean_up_debug_helpers() {
    rm -rf "${BASE_HELPER_FILE_DIR}" 2> /dev/null
}
